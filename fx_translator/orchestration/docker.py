...
class Orchestrator:
    ...
    def __init__(
        self,
        huridocs_image: str = "huridocs/pdf-document-layout-analysis:v0.0.24",
    ...
    def is_running(self) -> bool:
        ...
        code, out, err = run_cmd(
            ["docker", "ps", "--filter", f"name={self.huridocs_container}", "--format", "{{.Names}}"],
            timeout=10
        )
    ...
    def get_container_logs(self, tail: int = 50) -> str:
        ...
        """
        code, out, err = run_cmd(
            ["docker", "logs", "--tail", str(tail), self.huridocs_container],
            timeout=30
        )
