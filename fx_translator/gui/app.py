...
class AppGUI:
    ...
    def __init__(self, master: tk.Tk):
        ...
        self.huridocs_image = tk.StringVar(
            value="huridocs/pdf-document-layout-analysis:v0.0.24"
        )
        ...
        # LM Studio параметры
        self.lms_base = tk.StringVar(value=DEFAULT_LMS_BASE)
        self.lms_model = tk.StringVar(value=LMS_MODEL)
    ...
    def _build_ui(self):
        ...
        ttk.Label(frm, text="Исключения (стр.):").grid(
            ...
            if mode == "pymupdf":
                ...
                if transactional:
                    ...
                    run_pipeline(
                        ...
                        huridocs_base=self.huridocs_base.get(),
                        huridocs_analyze_path=self.huridocs_analyze_path.get(),
                        huridocs_visualize_path=self.huridocs_visualize_path.get() or None,
                        lms_base=self.lms_base.get(),
                        lms_model=self.lms_model.get(),
                        split_spreads_enabled=split_spreads_enabled,
                        batch_size=batch_size,
                        page_limit=self.page_limit.get() if self.page_limit.get() else None,
                        pause_ms=0,
                        force_split_exceptions=self.force_split_excl.get(),
                        force_split_spreads=bool(self.force_split.get()),
                        pause_hook=self.wait_if_paused,
