...
def _merge_ok(
    ...
    Args:
        ...
        x_tol_left: Допустимое отклонение по левой границе
        x_tol_right: Допустимое отклонение по правой границе
        gap_tol: Допустимый вертикальный зазор
    ...
    left_close = abs(a.left - b.left) <= max(x_tol_left, 0.01 * page_w)
    right_close = abs(a_right - b_right) <= max(x_tol_right, 0.02 * page_w)
    ...
    if vgap > max(gap_tol, 0.012 * a.pageheight):
        return False
...
def deglue_pages_pdfaware(pages: List[PageBatch], pdfpath: str) -> List[PageBatch]:
    """
