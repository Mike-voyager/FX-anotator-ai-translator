...
class AdvancedTextProcessor:
    ...
    def _should_merge_lines(
        ...
        font_size_ratio = abs(current_line.font_size - prev_line.font_size) / body_font_size
    ...
    def _classify_block_type(
        self, block: TextBlock, body_font_size: float
    ) -> str:
        """
        ...
        if avg_font_size > body_font_size * 1.3 and len(text) < 150:
            if text.isupper() or (text[0].isupper() and len(text.split()) <= 8):
                return "title" if avg_font_size > body_font_size * 1.6 else "section_header"
